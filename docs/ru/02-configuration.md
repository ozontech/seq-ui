...
Конфигурация задается `yaml`-файлом и переменными окружения.
`yaml`-файл состоит из 3 разделов:
...
> Устарело. Используйте `seq_db.addrs` вместо этого.

**`proxy_client_mode`** *`string`* *`default="grpc"`* *`options="grpc"`*

Способ отправки запросов в seq-db.

> Устарело. Используйте `seq_db.client_mode` вместо этого.
...
> Устарело. Используйте `seq_db.timeout` вместо этого.
...
> Устарело. Используйте `seq_db.avg_doc_size` вместо этого.
...
> Устарело. Используйте `seq_db.request_retries` вместо этого.
...
> Устарело. Используйте `seq_db.initial_retry_backoff` вместо этого.
...
> Устарело. Используйте `seq_db.max_retry_backoff` вместо этого.

**`grpc_keepalive_params`** *`GRPCKeepaliveParams`* *`optional`*

Параметры keepalive gRPC-клиента seq-db.

> Устарело. Используйте `seq_db.grpc_keepalive_params` вместо этого.
...
+ **`request_retries`** *`int`* *`default=0`*
  ...
  Количество повторных попыток отправки запроса клиенту после первой попытки. Для каждой повторной попытки будет выбран случайный адрес из списка `seq_db.addrs`. Нулевое значение означает отсутствие повторных попыток. Если установлено отрицательное значение, то оно будет сброшено на `default`.
...
+ **`default_target_bucket_rate`** *`string`* *`default="0s"`*
  ...
  Единица измерения к которой нормализуются значения бакетов в аггрегациях с таймсерией, если она не задана в поисковом запросе. Изначальная единица измерения бакета равна `count/interval`, после нормализации равна `count/target_bucket_rate`. Если установлено отрицательное значение, то оно будет сброшено на `default`. По умолчанию нормализация отключена.

  > Используется только в `count` аггрегациях. Значение должно быть передано в `duration`-формате: `<число>(ms|s|m|h)`.
...
+ **`masking`** *`Masking`* *`optional`*
  ...
  + **`default_env`** *`string`* *`optional`*
    ...
    Имя окружения, используемого по умолчанию для обработки запросов, когда явное указание окружения отсутсвует.
...
+ **`envs`** *`map[string]SeqAPIEnv`* *`optional`*
  ...
  + **`options`** *`SeqAPIOptions`* *`optional`*
    ...
    Специфичные для окружения параметры SeqAPI. Эти параметры замещают соответствующие глобальные настройки из корневой секции `seq_api`.
...
+ **`super_users`** *`[]string`* *`optional`*
  ...
  Список пользователей с полным доступом к административным функциям. Считаются суперадминистраторами и получают полный доступ независимо от ролей в БД. Полезно при первом запуске системы, когда в БД еще нет информации о пользователях, или при недоступности БД.
...
+ **`TRACING_AGENT_HOST`** *`string`* *`required`*
  ...
  Адрес хоста агента трейсинга (например, Jaeger).

+ **`TRACING_AGENT_PORT`** *`string`* *`required`*

  Порт агента трейсинга.
